...
class tank:
    ...
    def set_speedDC(self, joy):
		#Arm base
        self.Arm_DC(joy.x,joy.y, 129, self.speedDC)	
		#Yaw Wrist 
        self.Arm_DC1(joy.z, joy.w, 129, self.speedDC)
...
class servo:
    def __init__(self):
        self.ser = serial.Serial(port='/dev/ttyAMA0', baudrate=9600, parity=serial.PARITY_NONE, stopbits=serial.STOPBITS_ONE,bytesize=serial.EIGHTBITS,timeout=1)
                
    def Arm_servo(self, servo):
		
       if servo[0] == 1 and servo[1] == 0:
				self.ser.write('a')
				print "izqW"

       elif servo[0] == 0 and servo[1] == 1:
